...
mli_formula <- as.formula(paste("status_incl ~", paste(iv_vars, collapse = " + ")))
mli_model <- multinom(mli_formula, data = final_data_incl, weights = weight)
summary(mli_model)
...
# estimate ML2 model
ml_formula_ml2 <- as.formula(paste("status_incl2 ~", paste(iv_vars, collapse = " + ")))
ml_model_ml2 <- multinom(ml_formula_ml2, data = final_data_incl2, weights = weight)
summary(ml_model_ml2)

# Extract p-values for filtering significant predictors for BP2
ml_coef_ml2 <- summary(ml_model_ml2)$coefficients
ml_se_ml2   <- summary(ml_model_ml2)$standard.errors
